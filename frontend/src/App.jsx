...
export default function MainApp() {
  ...
  // --- HYPER-ACCURATE "JARVIS" AI ENGINE ---
  const handleSendAi = async () => {
    ...
    try {
        const availableCols = info.cols ? info.cols.join(", ") : "None";
        const chatTranscript = newChat.slice(-6, -1).map(m => `${m.role === 'ai' ? 'AI' : 'USER'}: ${m.text}`).join("\n");

        // --- NEW: THE PERFECT PERSONA PROMPT ---
        const prompt = `You are Sentinel, an advanced AI data assistant controlling a React Dashboard.
Available Dataset Columns: ${availableCols}

Conversation History:
${chatTranscript}

YOUR CAPABILITIES & RULES:
1. Speak naturally and intelligently. If the user says "hi", "hello", or "how are you", greet them warmly in the "text" field.
2. If the user asks a question about the data (e.g., "who won the most"), you must realize you do not have the raw data rows to answer them directly. Instead, setup the dashboard so THEY can see the answer. Set "view" to "dash", pick the relevant "column" (e.g., "winner"), and set "chart" to "bar". 
3. NEVER say generic things like "Here is the chart for match_id". Be highly specific and professional. Say: "I've plotted the match winners on a bar chart so we can see the exact statistics."
4. If they ask about a physical location, set "view" to "map" and extract the city to "location".
5. YOU MUST OUTPUT ONLY A VALID JSON OBJECT.

EXAMPLES OF PERFECT BEHAVIOR:
User: "Hi Sentinel"
Output: {"text": "Hello! I am online and ready. What data would you like to explore today?", "view": null, "location": null, "column": null, "chart": null, "export": false}

User: "Who was the best player?"
Output: {"text": "I have loaded the Player of the Match data into a bar chart so we can easily compare the top performers.", "view": "dash", "location": null, "column": "player_of_match", "chart": "bar", "export": false}

User: "Take me to Delhi on the map"
Output: {"text": "Zooming into Delhi on the interactive map now.", "view": "map", "location": "Delhi", "column": null, "chart": null, "export": false}

Now, handle this exact request:
...
`;
        ...
        for (let i = 0; i < GOOGLE_KEYS.length; i++) {
            if (success) break;
            try {
                const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GOOGLE_KEYS[i]}`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }] })
                });

                if (!response.ok) throw new Error("Google Failed");
                const data = await response.json();
                aiResponseText = data.candidates?.[0]?.content?.parts?.[0]?.text || "";
                success = true; 
            } catch (err) {
                console.warn(`Google Key ${i + 1} Failed.`);
            }
        }
        ...
        // --- STAGE 4: OFFLINE MODE (Fully Natural AI Persona) ---
        if (!success || !aiDecision) {
            aiDecision = { text: "I've processed your request and updated the interface.", view: null, location: null, column: null, chart: null, export: false };

            // 1. Conversational Greetings (No rigid robotic replies)
            const isGreeting = ['hi', 'hello', 'hey', 'sup', 'yo', 'greetings', 'hii', 'hiii'].some(g => userMessageLower === g || userMessageLower.startsWith(g + ' '));
            
            if (isGreeting) {
                aiDecision.text = "Hello! I am online and ready. What data would you like to explore today?";
            }
            ...
            // 5. Intelligent Dashboard Graphing (Deep Column Match)
            else {
                ...
                aiDecision.text = `I have analyzed the dataset and generated a ${aiDecision.chart} chart displaying the ${prettyName} statistics for you.`;
            }
        }

        // --- EXECUTE DASHBOARD CONTROL ---
        ...
        // Force graph view if requested or if analyzing data
        ...
        if (currentView === 'dash' || !aiDecision.view) {
            ...
            // Validate the chosen column against the real dataset columns
